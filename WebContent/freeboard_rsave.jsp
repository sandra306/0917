...
<%
 ...
 if (cont.length()==1) 
  ...
  try {
   ...
   sql = "select max(id) from  20162422_board";
   ...
   if (step == 1) {
    sql = "select max(replynum) from 20162422_board where masterid="+mid;
    rs=st.executeQuery(sql);
    if (!(rs.next())) 
     rnum=1;
    else 
   ...
   sql= "insert into 20162422_board(id,name,password,email,subject," ;
